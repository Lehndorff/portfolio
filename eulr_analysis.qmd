...
#Step 2 drop incomplete site ids NONE TO DROP
# site_agg<-analysis_data %>% 
#   group_by(ee_site_id) %>% 
#   summarise(n=n_distinct(paste(date,hour)))
...
A summary table of each end use including 2019 and 2020 average daily usage (between March 15 and September 1 of each year), percentage change and this distribution of daily usage.
...
initial_table<-final_dataset %>% 
  ...
  summarise(
    kWh=round(mean(kwh),2)
    ) %>% 
  pivot_wider(names_from = Year,values_from = kWh) %>% 
  mutate(`Percent Change`=round((`2020`-`2019`)/`2019`*100,2)) %>% 
...
text_transform(
 ...
 fn = function(Histogram) {
   map(initial_table$Histogram, ggplot_image, height = px(200))
 }
) %>% 
