...
```{r, echo=F, message=FALSE, warning=F,fig.height=10, fig.width=8}
...
Alternatively, we can focus on the journey of one individual -- the bird that starts farthest to the left.

```{r, echo=F, message=FALSE, warning=F,fig.height=10, fig.width=8}

example_bird=first(all_birds2$bird[all_birds2$x==max(all_birds2$x)])

bird_plot<-ggplot(all_birds2 %>% arrange(iteration) %>% filter(bird==example_bird))+
  # geom_point(aes(x=x,y=y,color=factor(bird),group=factor(bird),alpha=iteration))+
  geom_spoke(aes(x=x,y=y,color=factor(bird),group=factor(bird),angle=a2),radius=1,arrow=arrow(type = "closed",length = unit(3/range,"inches")))+
  guides(color='none')+
  xlim(c(min(all_birds$x),max(all_birds$y)))+
